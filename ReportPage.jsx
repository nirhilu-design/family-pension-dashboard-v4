...
function PrintReportA4({ reportData, recommendations = "" }) {
  ...
  const css = `
    ...
    @media print {
      ...
      .print-list-label { min-width: 0; display: flex; align-items: center; gap: 1.6mm; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .print-list-name { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: #102A43; font-weight: 800; }
  ...
  const PieBreakdown = ({ title, items, large = false, compact = false }) => {
    ...
    const isPercentOnlyPie = compact || total <= 100;
    const displayValue = (value) => isPercentOnlyPie ? `${Math.round(Number(value || 0))}%` : fmtCurrency(value);
    const totalDisplay = isPercentOnlyPie ? `${Math.round(total)}%` : fmtCurrency(total);
      ...
      <div className="print-card" style={{ minHeight: large ? "188mm" : compact ? "58mm" : "78mm" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: `${pieSize} minmax(0, 1fr)`, gap: "4mm", alignItems: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "2mm", minWidth: 0 }}>
            ...
            <div style={{ color: "#00215D", fontSize: large ? 13 : 11.5, fontWeight: 900, direction: "ltr" }}>{totalDisplay}</div>
          </div>
          <div style={{ minWidth: 0 }}>
